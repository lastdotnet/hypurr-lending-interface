...
>(({ className, ...rest }, ref) => (
  <thead
    ref={ref}
    className={clsx(
      'sticky top-0 z-10 outline outline-2 outline-zinc-100 md:outline-4',
      className
    )}
    {...rest}
  />
...
export const TableRow = forwardRef<HTMLTableRowElement, TableRowProps>(
  ({ bordered = true, className, ...rest }, ref) => (
    <tr
      ref={ref}
      className={clsx(
        'rounded-sm bg-white transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted',
        { 'outline outline-1 outline-gray-200': bordered },
        className
      )}
      {...rest}
    />
