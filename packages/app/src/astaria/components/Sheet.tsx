...
const sheetVariants = cva(
  'fixed z-50 gap-4 bg-background transition ease-in-out focus:outline-none data-[state=closed]:duration-300 data-[state=open]:duration-500 data-[state=open]:animate-in data-[state=closed]:animate-out',
  {
    defaultVariants: {
      mobileNav: false,
      side: 'left',
...
>(({ children, className, mobileNav, side = 'right', ...rest }, ref) => (
  <SheetPortal>
    ...
    <SheetPrimitive.Content
      ref={ref}
      className={clsx(sheetVariants({ className, mobileNav, side }))}
      {...rest}
    >
      {children}
      {mobileNav ? null : (
