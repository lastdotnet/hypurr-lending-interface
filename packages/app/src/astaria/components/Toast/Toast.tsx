...
>(({ className, ...rest }, ref) => (
  <ToastPrimitives.Viewport
    ref={ref}
    className={clsx(
      'fixed top-0 z-40 flex max-h-screen w-full flex-col-reverse gap-2 p-4 sm:bottom-0 sm:right-0 sm:top-auto sm:flex-col md:max-w-[420px]',
      className
    )}
    {...rest}
  />
...
>(({ className, tone, ...rest }, ref) => (
  <ToastPrimitives.Root
    ref={ref}
    className={clsx(toastVariants({ className, tone }))}
    {...rest}
  />
));
...
>(({ className, ...rest }, ref) => (
  <ToastPrimitives.Close
    ref={ref}
    className={clsx(
      'absolute -right-0.5 -top-0.5 flex h-11 w-11 items-center justify-center rounded-md p-1 text-foreground hover:text-foreground focus:outline-none focus:ring-2 group-[.destructive]:text-red-300 group-[.destructive]:hover:text-red-50 group-[.destructive]:focus:ring-red-400 group-[.destructive]:focus:ring-offset-red-600',
      className
    )}
    toast-close=""
    {...rest}
