...
export interface SendSuccessViewProps {
  sent: TokenWithValue[]
  toAddress: string
  runConfetti: boolean
  resetForm: () => void
...
export function SendSuccessView({ sent, toAddress, runConfetti, resetForm }: SendSuccessViewProps) {
  ...
  return (
    <PageLayout className="pt-8 xl:pt-28">
...
function Address({ address }: { address: string }) {
  return (
    <div className="flex items-center border-t">
      <div className="flex grow flex-col py-3 pr-4">
        <Typography className="font-primary">{address}</Typography>
      </div>
    </div>
  )
}
