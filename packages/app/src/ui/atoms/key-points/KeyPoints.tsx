...
function KeyPointsItem({ children, variant }: { children: ReactNode; variant: 'positive' | 'negative' }) {
  return (
    <li className="flex items-center gap-2.5 text-basics-blacks">
      {variant === 'positive' && <Image src={assets.success} alt="positive-checkmark-icon" className="h-5" />}
      {variant === 'negative' && <Image src={assets.closeFilled} alt="negative-cross-icon" className="h-5" />}
      {children}
    </li>
  )
}
