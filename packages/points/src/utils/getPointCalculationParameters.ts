...
import { DAILY_BONUS_POINTS, INTENT_TRANSMISSION_BONUS_POINTS } from '../constants'
import { calculatePointsForPoint, getTokenAmount } from './calculatePointsForPoint'
...
export const getPointCalculationParameters = ({
  ...
  const totalNonDynamicPoints = points
    ...
    .map((point) => {
      ...
      if (point.event === 'IntentSubmission') {
        return numberToBigInt({
