...
const getLogoURI = ({
  ...
  const fileExists = existsSync(
    `data/images/erc20/${symbol.toLowerCase()}.svg`
  );
  if (fileExists) {
  ...
  if (
    logoURI &&
    !logoURI.startsWith('https://assets.coingecko.com/coins/images')
  ) {
    throw new Error(
...
const cleanTokens = combinedTokens.map(
...
writeFileSync(
  'src/constants/erc20Tokens.json',
...
const supportedSymbolsMainnetEnum = convertToEnum(
  getSupportedSymbols(mainnet.id)
);
const exportSymbolsMainnet = `export enum SupportedSymbolsMainnet ${supportedSymbolsMainnetEnum};`;
const supportedSymbolsSepoliaEnum = convertToEnum(
  getSupportedSymbols(sepolia.id)
);
