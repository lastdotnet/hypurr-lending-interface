...
const getLookupTable = (chainId: ChainId) =>
  getERC20TokensByChainId(chainId).reduce((obj, token) => Object.assign(obj, { [token.symbol]: token }), {})
...
export const getERC20TokenBySymbol = ({ chainId, symbol }: GetERC20TokenBySymbolProps): ERC20Asset => {
  if (chainId === base.id) {
