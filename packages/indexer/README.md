...
- Note: You must run deployment command from the `services/indexer` directory
- Run `pnpm deploy:production` or `pnpm deploy:preview` to deploy, overwriting the existing indexer at v1
...
generated by the squid framework from `schema.graphql`. This is done by running `pnpm codegen`
command.
...
It is necessary to import the respective ABI definition to decode EVM logs. One way to generate a type-safe facade class to decode EVM logs is by placing the relevant JSON ABIs to `./abi`, then using `squid-evm-typegen(1)` via a script:
