...
export type LoanOpenArgs = Required<
  GetEventArgs<typeof Events, 'Open', { IndexedOnly: false }>
>;

export async function isValidStarportLoan(
  publicClient: PublicClient,
  loan: LoanOpenArgs['loan']
): Promise<boolean> {
  ...
  if (errors.length > 0) {
    console.warn(
      `StarportLoan ${JSON.stringify(loan)} is invalid: ${errors.join(';')}`
    );
    return false;
  }
