...
export const getIntentFillStartBlock = ({ chainId }: { chainId: ChainId }) => intentFillStartBlockLookup[chainId]

export const getStartBlock = ({ chainId }: { chainId: ChainId }) => {
  //if forking, use anvil start block
  if (ENV.FORK_URL) {
    return Number.parseInt(readFileSync('./src/anvil-start-block.txt').toString()) + 1
  }
