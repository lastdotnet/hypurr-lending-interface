...
describe('formatDuration', () => {
  ...
  describe('formatDuration', () => {
    ...
    it('should handle pluralization', () => {
      ...
      expect(
        formatDuration({ milliseconds: milliseconds({ days: 1 }) })
      ).toEqual('1d');
      expect(
        formatDuration({ milliseconds: milliseconds({ days: 5 }) })
      ).toEqual('5d');
    });
    it('should handle durations over DAYS_CUTOFF correctly', () => {
      expect(
        formatDuration({ milliseconds: milliseconds({ days: 2.25 }) })
      ).toEqual('2d 6h');
      expect(
        formatDuration({ milliseconds: milliseconds({ days: 14 }) })
      ).toEqual('14d');
    });
