...
describe('secondsFromNow', () => {
  ...
  it('should return the correct number of seconds', () => {
    const timestampInSeconds = millisecondsToSeconds(
      currentTimestamp() + getSeconds({ seconds: 15 })
    );

    expect(secondsFromNow(timestampInSeconds)).toBeCloseTo(
      getSeconds({ seconds: 15 })
    );
  });
