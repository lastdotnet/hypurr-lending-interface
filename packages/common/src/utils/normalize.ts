...
const removeUnsafeDigits = (string: string) => removeDigits({ digits: JS_MAX_SAFE_SIGNIFICANT_DIGITS, string })

export const toNormalizedValue = (value = 0n, decimals = ETHER_DECIMALS): number => {
  const formatted = formatUnits(value, decimals)

  return Number(removeUnsafeDigits(formatted))
}

export const toNormalizedString = (value = 0n, decimals = ETHER_DECIMALS) => formatUnits(value, decimals)
